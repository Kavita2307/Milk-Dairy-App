...
//   }
// };
export const dailyFeedEfficiencyReport = async (
  req: Request,
  res: Response
) => {
  ...
  try {
    ...
    ----------------------------------- */
    const totalHerdStrength = await prisma.animal.count({
      where: { userId },
    });
    ...
    const animalsByGroup: Record<number, number> = {};
    ...
    /* -----------------------------------
       3. GROUP WISE MILK
    ----------------------------------- */
    const milkData = await prisma.milk.groupBy({
      by: ["groupId"],
      where: {
        userId,
    ...
    const milkByGroup: Record<number, number> = {};
    milkData.forEach((m) => {
    ...
    ----------------------------------- */
    const tmrData = await prisma.ration.groupBy({
      by: ["groupId"],
      where: {
        userId,
    ...
    const tmrByGroup: Record<number, number> = {};
    tmrData.forEach((t) => {
    ...
    /* -----------------------------------
       5. LEFTOVER
    ----------------------------------- */
    const leftoverData = await prisma.leftover.groupBy({
      by: ["groupId"],
      where: {
        userId,
        date: { gte: start, lte: end },
    ...
    const leftoverByGroup: Record<number, number> = {};
...
export const dailyAverageDmiReport = async (req: Request, res: Response) => {
  try {
    const userId = Number(req.query.userId);
    ...
    const animalCount: Record<number, number> = {};
    animals.forEach((a) => {
...
export const cowWiseMilkReport = async (req: Request, res: Response) => {
  try {
    const userId = Number(req.query.userId);
    const animalNumber = req.query.cowNo as string;
    ...
    /* -----------------------------
       MILK DATA (DATE-WISE)
    ------------------------------ */
    const milkRows = await prisma.milk.groupBy({
      by: ["date"],
      where: {
        animalNumber,
    ...
    const avg7Day =
      rows.length > 0 ? Number((totalMilk / rows.length).toFixed(1)) : 0;
...
export const getCowNumbers = async (req: Request, res: Response) => {
  try {
    const userId = Number(req.query.userId);
...
export const groupWiseDailyMilkAvgReport = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    milkRows.forEach((m) => {
      ...
      if (!report[dateKey]) report[dateKey] = {};
    ...
    /* -----------------------------
       7 Day Average
    ------------------------------ */
    const avgRow: any = { date: "7 Day Avg" };
